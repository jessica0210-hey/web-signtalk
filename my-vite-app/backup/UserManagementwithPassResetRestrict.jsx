...
function UserManagement() {
  ...
  const fetchUsersWithCurrentUserDetection = async () => {
    ...
    const usersArr = snapshot.docs.map((doc, idx) => {
      ...
      // TEMPORARY SOLUTION: Check for lastLoginMethod === 'google' to identify Google sign-in users
      // TODO: Find proper solution for Google sign-in password management once available
      const hasGoogleAuth = data.lastLoginMethod === 'google';
      console.log('User data:', { formatted_uid: data.formatted_uid, uid: data.uid, doc_id: doc.id, final_id: userIdString, isCurrentUser, lastLoginMethod: data.lastLoginMethod, hasGoogleAuth });
      return {
        id: userIdString,
        docId: doc.id,
        ...
        hasGoogleAuth: hasGoogleAuth  // Flag for users with Google authentication
      };
    });
  ...
  const handleResetPassword = (user) => {
    // TEMPORARY SOLUTION: Block password reset for Google sign-in users
    // TODO: Find proper solution for Google sign-in password management once available
    if (user.hasGoogleAuth) {
      showError('Password reset is not available for Google sign-in accounts. This user authenticates through Google and we do not have authority to reset their Google password.');
      console.log('Reset password blocked for Google sign-in user (lastLoginMethod: google):', user);
      return;
    }
                    ...
                    (activeTab === "users" ? filteredUsers : filteredAdmins).map((item, i) => (
                      <tr key={item.id || i} style={styles.row}>
                        ...
                        <td style={{ ...styles.td, borderTopRightRadius: "10px", borderBottomRightRadius: "10px", textAlign: "right", width: '25%' }}>
                          {/* TEMPORARY SOLUTION: Disable reset password for Google sign-in users */}
                          {/* TODO: Remove visual disabling once proper solution is found */}
                          <button id={`reset-btn-${item.id || i}`}  
                            style={{
                              ...styles.resetBtn,
                              opacity: item.hasGoogleAuth ? 0.5 : 1,
                              cursor: item.hasGoogleAuth ? 'not-allowed' : 'pointer',
                              backgroundColor: item.hasGoogleAuth ? '#999' : '#6F22A3'
                            }}
                            onClick={() => handleResetPassword(item)}
                            onMouseEnter={(e) => {
                              if (!item.hasGoogleAuth) {
                            ...
                            title={item.hasGoogleAuth ? 'Password reset disabled for Google sign-in accounts' : 'Reset user password'}
                          >
                            Reset Password
                          </button>
