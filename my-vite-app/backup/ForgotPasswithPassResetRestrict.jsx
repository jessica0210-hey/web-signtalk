...
function LoginWrapper() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // TEMPORARY SOLUTION: Check if user uses Google Sign-In
      // TODO: Find a proper solution for managing Google sign-in user passwords once available
      // Users with lastLoginMethod: "google" authenticate via Google and cannot reset password through this flow
      const userDoc = querySnapshot.docs[0];
      const userData = userDoc.data();
      
      if (userData.lastLoginMethod === 'google') {
        setLoading(false);
        setPopupType('error');
        setPopupMessage('This account uses Google Sign-In authentication. Password reset is not available for Google accounts. Please sign in with Google.');
        setShowPopup(true);
        console.log('User uses Google sign-in (lastLoginMethod: google), password reset disabled:', email);
        return;
      }
